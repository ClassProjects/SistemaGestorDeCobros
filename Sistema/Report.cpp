...
#include <stdio.h>
	...
	void Report::PrintReportClient(string pClientID, string pInial_Date, string pEnd_Date){
		ClientTotal = 0;
	...
	void Report::open(){
		reportstream.open("./reports/ReportsL.txt");
	}

	double* Report::getCallTime(string pStart,string pEnd, string type){
		double* pointer;
		double info[1];
		pointer = info;
		int Minutes = 0;
